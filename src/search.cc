...
void Search::init_LMR_array(SearchParms sp){
  ...
  for (int depth = 0; depth < MAX_PLY; depth++){
    _lmp_Array[depth][0] = (int)  (LMP_START_BASE + pow((depth - 1), 2) * LMP_MULTIPL_BASE);
    _lmp_Array[depth][1] = (int)  (LMP_START_IMPR + pow((depth - 1), 2) * LMP_MULTIPL_IMPR);
  }
...
int Search::_negaMax(Board &board, pV *up_pV, int depth, int alpha, int beta, bool singSearch, bool cutNode) {
  ...
  if (isPrune && pMove != 0 && nodeEval >= beta + std::max(0, NMP_COND_BASE - NMP_COND_DEPTH * depth) && board.isThereMajorPiece()){
          Board movedBoard = board;
          _posHist.Add(board.getZKey().getValue());
          _sStack.AddNullMove(getOppositeColor(board.getActivePlayer()));
          movedBoard.doNool();

          int fDepth = depth - NMP_BASE - depth / NMP_DEPTHDIV - std::min((nodeEval - beta) / NMP_DELTA_DIV, NMP_MAXREDUCT);
      ...
      if (depth <= 3 && isQuiet && cmHistory <= ( CMH_DEPTH * depth + CMH_BASE)) continue;
        ...
        if (ttEntry.Flag != ALPHA &&
            ...
            abs(ttEntry.score) < WON_IN_X / 4){
              ...
              if (sBeta > score){
                tDepth += 1 + (!pvNode && depth > SING_SEARCH_START);
                singNode = true;
