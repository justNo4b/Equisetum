...
void Search::init_LMR_array(){
  ...
  for (int depth = 0; depth < 34; depth++){
    for (int movenum = 0; movenum < 34; movenum++){
      _lmr_R_array[0][depth][movenum] = (int) (0.26 + (pow(depth, 0.12) * pow(movenum, 0.16))/1.77);
      _lmr_R_array[1][depth][movenum] = (int) (0.54 + (pow(depth, 0.14) * pow(movenum, 0.19))/1.35);
    }
  }
  // 2. Initialization of the LMP array.
  // Current formula is completely based on the Weiss chess engine.
  for (int depth = 0; depth < MAX_PLY; depth++){
    _lmp_Array[depth][0] = (int)  (1.59 + pow((depth - 1), 2) * 1.75);
    _lmp_Array[depth][1] = (int)  (3.59 + pow((depth - 1), 2) * 2.19);
  }
...
inline void Search::_updateBeta(bool isQuiet, const Move move, Color color, int pMove, int ply, int depth){
    ...
    _orderingInfo.incrementCounterHistory(color, pMove, move.getPieceType(), move.getTo(), depth, 6);
  }else{
...
int Search::_negaMax(Board &board, pV *up_pV, int depth, int alpha, int beta, bool singSearch, bool cutNode) {
  ...
  if (isPrune && depth <= 6 && ((nodeEval - 148 * depth + 148 * improving) >= beta)){
      return beta;
  }
  ...
  if (isPrune && pMove != 0 && nodeEval >= beta + std::max(0, 126 - 21 * depth) && board.isThereMajorPiece()){
          Board movedBoard = board;
          _posHist.Add(board.getZKey().getValue());
          _sStack.AddNullMove(getOppositeColor(board.getActivePlayer()));
          movedBoard.doNool();

          int fDepth = depth - 4 - depth / 5 - std::min((nodeEval - beta) / 178, 6);
  ...
  if (!pvNode &&
       depth >= 5 &&
       alpha < WON_IN_X){
        int pcBeta = beta + 219 - 116 * improving;;
        while (movePicker.hasNext()){
            Move move = movePicker.getNext();
  ...
  while (movePicker.hasNext()) {
    ...
    if (isLegal){
        ...
        if (doLMR){
          ...
          reduction -= moveHistory / 8181;
          reduction -= cmHistory  / 7901;
...
int Search::_qSearch(Board &board, int alpha, int beta) {
  ...
  while (movePicker.hasNext()) {
    ...
    if (!(move.getFlags() & Move::PROMOTION) && !board.SEE_GreaterOrEqual(move, (alpha - standPat - 166)))
      continue;;
