...
int Search::_getHistoryPenalty(int depth, int eval, int alpha, int pmScore, bool ttNode, bool cutNode, bool pvNode){
  ...
  while (movePicker.hasNext()) {
    ...
    if (isLegal){
        ...
        if (score > alpha) {
          ...
          int penalty = _getHistoryPenalty(depth, nodeEval, alpha, pMoveScore, ttNode, cutNode, pvNode);
          if (isQuiet){
